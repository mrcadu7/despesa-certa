...
def test_update_expense_owner():
    ...
    data = {'value': 99.99, 'category': 'Alimentação', 'date': date.today(), 'description': 'Editado'}
...
def test_update_expense_not_owner():
    ...
    expense = Expense.objects.create(user=owner, value=10, category='Alimentação', date=date.today())
    client = APIClient()
    client.force_authenticate(user=other)
    data = {'value': 50, 'category': 'Alimentação', 'date': date.today(), 'description': 'Hack'}
    response = client.put(f'/api/expenses/{expense.id}/', data, format='json')
